...
class AddressLookupTable {
  ...
  @override
  int get hashCode => Object.hash(discriminator, deactivationSlot, lastExtendedSlot, lastExtendedSlotStartIndex, authority, padding, addresses);

  @override
  String toString() => 'AddressLookupTable(discriminator: $discriminator, deactivationSlot: $deactivationSlot, lastExtendedSlot: $lastExtendedSlot, lastExtendedSlotStartIndex: $lastExtendedSlotStartIndex, authority: $authority, padding: $padding, addresses: $addresses)';
}
...
Encoder<AddressLookupTable> getAddressLookupTableEncoder() {
  final structEncoder = getStructEncoder(<(String, Encoder<Object?>)>[
    ...
    ('authority', getNullableEncoder<Address>(getAddressEncoder(), noneValue: const ZeroesNoneValue())),
    ('padding', getU16Encoder()),
    ('addresses', getArrayEncoder(getAddressEncoder(), size: RemainderArraySize())),
  ]);
...
Decoder<AddressLookupTable> getAddressLookupTableDecoder() {
  final structDecoder = getStructDecoder(<(String, Decoder<Object?>)>[
    ...
    ('authority', getNullableDecoder<Address>(getAddressDecoder(), noneValue: const ZeroesNoneValue())),
    ('padding', getU16Decoder()),
    ('addresses', getArrayDecoder(getAddressDecoder(), size: RemainderArraySize())),
  ]);

  return transformDecoder(
    structDecoder,
    (Map<String, Object?> map, Uint8List bytes, int offset) => AddressLookupTable(
...
Account<AddressLookupTable> decodeAddressLookupTable(EncodedAccount encodedAccount) {
